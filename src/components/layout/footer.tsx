  ...
  return (
    <footer className="bg-background w-full">
      <div className="mx-auto max-w-7xl px-6 py-8 md:flex md:items-center md:justify-between md:px-8 lg:px-12 xl:px-16">
          ...
          {navigation.map((item) => (
            <div key={item.name}>
              ...
              >
                ...
                <item.icon
                  className={`${item.name === 'Telegram' || item.name === 'GitHub' ? '-mr-1 mb-0.5 h-7 w-7' : 'h-6 w-6'} text-foreground hover:text-primary`}
                  aria-hidden="true"
                />
              </a>
            </div>
        ...
        {!isHomePage && (
          <div className="md:min-w-1/3 mt-4 flex items-center justify-center gap-x-3 md:order-1 md:mt-0">
